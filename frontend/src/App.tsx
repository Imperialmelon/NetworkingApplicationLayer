...
import { hostname, type Message } from "./consts"
import { Chat } from "./components/Chat/Chat"
import { MarsChat } from "./components/MarsChat/MarsChat"

function App() {
  const { login } = useUser()
  const [ws, setWs] = useState<WebSocket | undefined>() // весокет
  const [messageArray, setMessageArray] = useState<Message[]>([])
  const [isMars, setIsMars] = useState<boolean>(false)

  // Check if user is Mars
  useEffect(() => {
    if (login === "Марс") {
      setIsMars(true)
    } else {
  ...
  return (
    <>
      <div className="App">
        {login ? (
          isMars ? (
            <MarsChat ws={ws} setWs={setWs} createWebSocket={createWebSocket} />
          ) : (
